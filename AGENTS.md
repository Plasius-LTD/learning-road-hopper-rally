...
### Local Notes
This repo is `@plasius/learning-road-hopper-rally`, the infrastructure-neutral Road Hopper Rally curriculum, deterministic runtime, asset, and assessment package.
...
### Repo conventions
- Source lives in `src/`; tests in `tests/`; original package assets live in `assets/`.
...
- Keep React, HTTP, authentication, persistence, entitlement evaluation, telemetry, and cloud dependencies out of this package.
- Keep protected assessment scenarios in the `./server` entry point and out of root/browser exports.
- Treat course, starter, asset, and assessment versions as immutable once published.
